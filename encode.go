...
func Encode(xComponents, yComponents int, img image.Image) (hash string, err error) {
	...
	xComponentsEncoded, err := base83.Encode(xComponents, 1)
	if err != nil {
		return "", err
	}
	yComponentsEncoded, err := base83.Encode(yComponents, 1)
	if err != nil {
		return "", err
	}

	_, err = b.WriteString(xComponentsEncoded)
	if err != nil {
		return "", err
	}

	_, err = b.WriteString(yComponentsEncoded)
	if err != nil {
		return "", err
	}
